	...
	$.ajax({
	       ...
	       success: function(tmp){
	    	 ...
	    	 //Adding image to page
	    	   ...
	    	   alert(tmp['product_discount']/100);

	    	   //Seller info
	    	   document.getElementById("seller").innerHTML = tmp['seller_name']+"( "+tmp['seller_rating']+" <span class='glyphicon glyphicon-star-empty'></span> )";

	    	   //Price
	    	   if(tmp['product_discount']>0){
	    		   $('#price').append("Rs. <strike>"+tmp['product_price']+"</strike>");
	    		  // $('#price_div').append("First");
	    		   $('#price_div').append("<strong><p style='text-align:left;margin-top:8px;font-size:22px'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+(tmp['product_price']-tmp['product_price']*(tmp['product_discount']/100))+"</p></strong>");
	    	   }else{
	    	   ...
	    	   $('#totalStar').append(" "+tmp['seller_total_rating_count']);
	    	   $('#totalUser').append(" "+tmp['seller_user_rating_count']);
